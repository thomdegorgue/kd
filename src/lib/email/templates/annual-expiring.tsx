interface AnnualExpiringEmailProps {
  ownerEmail: string
  storeName: string
  daysLeft: number
  billingUrl: string
...
export function AnnualExpiringEmail({
  ownerEmail,
  storeName,
  daysLeft,
  billingUrl,
}: AnnualExpiringEmailProps) {
  return `
<!DOCTYPE html>
<html lang="es">
<head>
  ...
  <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; padding: 40px; border: 1px solid #fde68a;">
    ...
    <div style="background: #fffbeb; border-left: 4px solid #fbbf24; padding: 16px; margin: 20px 0; border-radius: 6px;">
      <p style="color: #92400e; font-size: 15px; line-height: 1.6; margin: 0;">
        El plan anual de <strong>${storeName}</strong> vence en <strong>${daysLeft} día${daysLeft === 1 ? '' : 's'}</strong>.
      </p>
    </div>

    <p style="color: #666; font-size: 16px; line-height: 1.6; margin: 0 0 30px 0;">
      Renovalo con tiempo para evitar interrupciones.
    </p>
