  ...
  const PaymentPanel = (
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="space-y-2">
        ...
        {paymentMethod === 'savings' && savingsModuleActive && (
          <Select value={savingsAccountId} onValueChange={(v) => { if (v) setSavingsAccountId(v) }}>
            <SelectTrigger className="h-8 text-sm">
              <SelectValue placeholder="Seleccioná una cuenta" />
            </SelectTrigger>
            <SelectContent>
              {((savingsAccounts as unknown as SavingsAccount[]) ?? []).map((acc) => (
      ...
      <Button
        ...
        disabled={cart.length === 0 || isPending || (paymentMethod === 'savings' && !savingsAccountId)}
